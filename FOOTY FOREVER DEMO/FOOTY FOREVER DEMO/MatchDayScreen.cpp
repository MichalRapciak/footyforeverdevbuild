...
void MatchDayScreen::update(sf::Time dt, sf::RenderWindow& window) {
    ...
    if (!m_awayTeamId.empty()) {
        ...
        if (ImGui::BeginChild("AwayXIList", ImVec2(comboWidth, formationMapHeight), true)) {
            ...
            ImGui::BeginDisabled();
            ImGui::Button("AI Controlled Roster##AwayAuto", ImVec2(comboWidth - 15.f, 35));
            ImGui::EndDisabled();
            ImGui::Separator();
            ImGui::Spacing();
            ...
            for (size_t i = 0; i < formationLayout.size(); ++i) {
                ...
                for (size_t j = 0; j < line.size(); ++j) {
                    ...
                    ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(teamColor.x * 0.6f, teamColor.y * 0.6f, teamColor.z * 0.6f, 0.6f));
                    ImGui::PushStyleColor(ImGuiCol_ButtonActive, ImVec4(teamColor.x * 0.6f, teamColor.y * 0.6f, teamColor.z * 0.6f, 0.6f));

                    // LARGER BUTTON HEIGHT (55 instead of 40)
                    ImGui::Button((buttonText + buttonId).c_str(), ImVec2(buttonWidth, 55));
        ...
        if (ImGui::BeginChild("AwayBenchList", ImVec2(comboWidth, 0), true)) {
            ...
            for (auto& [benchPId, benchPlayer] : m_db->players) {
                ...
                if (std::find(startingIds.begin(), startingIds.end(), benchPId) == startingIds.end()) {
                    ...
                    // LARGER BENCH BUTTONS (35 instead of 25)
                    ImGui::Button(benchBtnLabel.c_str(), ImVec2(comboWidth - 20.f, 35));
